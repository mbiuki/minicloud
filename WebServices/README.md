...
| SendSensorDataToSlack | An IoT Rule invokes this Lambda function whenever a sensor publishes updated data to AWS IoT. Set the `SLACK_WEBHOOK` environment variable to the webhook URL from creating the Slack incoming webhook. | Not meant to be used as an API Gateway endpoint. |
